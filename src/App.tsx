...
function NewServiceModal({ user, serviceToEdit, onClose, onSuccess }: { user: UserProfile | null, serviceToEdit?: Service | null, onClose: () => void, onSuccess: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      ...
      const data = await res.json();
      console.error('Save service error:', data);
    ...
    >
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            className="w-full bg-blue-600 text-white py-4 rounded-xl font-bold shadow-lg shadow-blue-200 mt-4"
          >
            Save Service
          </button>
        </form>
...
function ServiceDetailModal({ service, user, onClose, onRate }: { service: Service, user: UserProfile | null, onClose: () => void, onRate: (rating: number) => void }) {
              ...
              <div className="flex justify-between items-center">
                ...
                {hasChanged && (
                  <button 
                    ...
                    className="text-[10px] bg-blue-600 text-white px-2 py-1 rounded-full font-bold"
                  >
                    Save Now
                  </button>
                )}
